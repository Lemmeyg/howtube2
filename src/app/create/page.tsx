...
function GuideEditorLoader({ url }: { url: string }) {
  ...
  useEffect(() => {
    ...
    if (!url || !user || pipelineInitiated.current) return
          ...
          try {
            ...
            if (
              ...
              (statusData.status.status === 'completed' ||
                (statusData.status.status === 'transcribing' &&
                  statusData.status.transcription_job_id) ||
                statusData.status.current_step === 'pipeline_complete')
            ) {
              if (
                statusData.status.transcription_job_id ||
                (statusData.metadata && statusData.metadata.transcription_id)
              ) {
                const transcriptionId =
                  statusData.status.transcription_job_id || statusData.metadata.transcription_id
                console.log('Polling transcription status for job ID:', transcriptionId)
                const transStatusRes = await fetch(
                  `/api/videos/transcribe?id=${id}&transcriptionId=${transcriptionId}`,
                  {
                    credentials: 'include',
                  }
                )
                ...
                if (!transStatusRes.ok) {
                  console.warn(
                    'Failed to get specific transcription status:',
                    transStatusData.error
                  )
                } else {
                ...
                console.warn(
                  "video_processing is 'completed' but no transcription ID found. Proceeding to guide generation if applicable or this might be an error."
                )
                clearInterval(polling)
                setStatus('generating')
                const guideRes = await fetch('/api/videos/guide', {
                ...
                if (!guideRes.ok) {
                  setError(
                    guideData.error ||
                      'Failed to generate guide (after completion without transcription ID)'
                  )
                  setStatus('error')
                  return
                }
